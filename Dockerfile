...
RUN wget -qO- "https://cmake.org/files/v3.14/cmake-3.14.1-Linux-x86_64.tar.gz" | tar --strip-components=1 -xz -C /usr
...
#
##install llvm and clang v8
#WORKDIR /tmp
#RUN git clone https://github.com/llvm-mirror/llvm.git
#WORKDIR /tmp/llvm
#RUN git checkout -b release_80 origin/release_80
#WORKDIR /tmp/llvm/tools
#RUN git clone https://github.com/cpc/hipcl-clang.git clang
#WORKDIR /tmp/llvm/tools/clang
#RUN git checkout -b release_80 origin/release_80
#WORKDIR /tmp/llvm/build
#RUN cmake -DCMAKE_INSTALL_PREFIX=/llvm /tmp/llvm
...
#
##TODO
##install hippify-clang (as a replacement for Coriander to convert CUDA->OpenCL) CUDA->Hip
##WORKDIR /tmp/llvm
##RUN git clone https://github.com/ROCm-Developer-Tools/HIPIFY.git
##WORKDIR /tmp/llvm/hipify/build
##RUN cmake -DCMAKE_INSTALL_PREFIX=/HIPPIFY -DLLVM_SOURCE_DIR=../llvm -DLLVM_TARGETS_TO_BUILD="X86;NVPTX"  -DCMAKE_BUILD_TYPE=Release ../llvm
##RUN make -j install
#
###install LLVM-SPIRV translator
...
##
###install HIPCL to convert Hip->OpenCL
